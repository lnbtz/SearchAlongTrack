	...
	<div class="results-card">
		...
		<div class="table-scroll">
			<table class="sat-table">
				...
				<tbody>
					{#each tableDataDisplay as row (row)}
						<tr>
							<td class="type-cell">
								<img
									class="type-icon"
									src={getCategoryIconUrl(row.category)}
									alt={displayType(row.type)}
								/>
								...
								<details class="show-more">
									...
									<div class="info-box details-grid">
										...
										<div>
											...
											{#if row.openingHours}
												<div>
													{#each row.openingHours.split(';') as segment, i (i)}
														{segment.trim()}{#if i < row.openingHours.split(';').length - 1}<br
															/>{/if}
													{/each}
												</div>
											{:else}
										...
										<div class="links-container">
											...
											>
												...
												>
													...
													></path><path
														fill="#3996e8"
														d="M18,6v17h-6V6.052C12.28,6.015,12.57,6,12.87,6H18z"
													></path><path
														fill="#1976d2"
...
<style>
	...
	.info-box {
		...
		top: 100%;
		left: 0;
		box-shadow: 0 1px 4px rgba(66, 103, 178, 0.1);
