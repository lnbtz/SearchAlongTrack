    ...
    <header class="results-head">
        <div class="results-title">
            <div class="title-left">
                ...
                <h3>Along your track</h3>
            </div>
            <div class="title-right">
                <span class="count-badge">{tableDataDisplay.length} Results</span>
            </div>
        </div>
        <p class="muted">Tap a row to show it on the map. Adjust filters in the map overlay.</p>
    </header>
    ...
    <table class="sat-table">
		...
		<tbody>
            {#each tableDataDisplay as row}
				<tr>
                    <td class="type-cell">
                        <img class="type-icon" src={getCategoryIconUrl(row.category)} alt={displayType(row.type)} />
                        <span class="type-text">{displayType(row.type)}</span>
                    </td>
                    <td class="name-cell">
                        <button class="name-btn" title="Show on map" onclick={() => handleShowOnMap(row)}>
                            {row.name || '—'}
                        </button>
                    </td>
                    <td>
                        <details class="show-more">
                            <summary class="clickable">Details</summary>
                            <div class="info-box details-grid">
                                <div>
                                    <strong>Website:</strong>
                                    {#if row.website}
                                        <a href={row.website} target="_blank" rel="noopener noreferrer">Open</a>
                                    {:else}
                                        <span>-</span>
                                    {/if}
                                </div>
                                <div>
                                    <strong>Phone:</strong>
                                    {#if row.phoneNumber}
                                        <a href={`tel:${row.phoneNumber}`} onclick={(e) => {
                                            e.preventDefault();
                                ...
                                <div class="links-container">
									...
									class="map-icon"
									onclick={() => {
										const coords = `${row.location?.lat},${row.location?.lon}`;
										navigator.clipboard.writeText(coords);
										alert('Coordinates copied to clipboard');
									}}
								>
									<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 20 20" fill="none">
										<rect x="5" y="3" width="10" height="14" rx="2" stroke="currentColor" stroke-width="2" fill="none"/>
										<rect x="3" y="5" width="10" height="14" rx="2" stroke="currentColor" stroke-width="2" fill="none" opacity="0.4"/>
									</svg>
								</button>
									<button
                    ...
                    <td><span class="metric">{row.distanceFromRoute} m</span></td>
                    <td><span class="metric">{row.distanceOnRoute} km</span></td>
				</tr>
			{/each}
        </tbody>
    </table>
...
<style>
    ...
    .details-grid { display: grid; gap: .5rem; }
    ...
    .results-title { display: flex; align-items: center; justify-content: space-between; gap: .75rem; }
    .results-head h3 { margin: 0; font-weight: 800; letter-spacing: .2px; }
    .results-head .muted { margin: .25rem 0 0 0; color: var(--text-muted); }
    .pill { display: inline-flex; align-items: center; gap: .4ch; font-weight: 700; font-size: .75rem; color: var(--primary-700); background: color-mix(in oklab, var(--primary) 15%, transparent); border: 1px solid color-mix(in oklab, var(--primary) 35%, transparent); padding: .15rem .5rem; border-radius: 999px; }
    .title-left { display: flex; align-items: center; gap: .5rem; }
    .title-right { display: inline-flex; align-items: center; gap: .5rem; }
    .count-badge { display: inline-grid; place-items: center; min-width: 28px; height: 28px; padding: 0 .4rem; border-radius: 999px; background: var(--bg); border: 1px solid var(--border); box-shadow: var(--shadow-sm); font-weight: 800; }
    ...
    .type-cell { display: inline-flex; align-items: center; gap: .5rem; }
    .type-icon { width: 18px; height: 18px; border-radius: 4px; box-shadow: var(--shadow-sm); }
    .name-btn { background: transparent; border: none; color: var(--primary-700); text-decoration: underline; cursor: pointer; font-weight: 600; }
    .metric { display: inline-flex; align-items: center; gap: .35rem; padding: .15rem .4rem; border-radius: 6px; background: var(--bg); border: 1px solid var(--border); box-shadow: var(--shadow-sm); font-weight: 700; font-variant-numeric: tabular-nums; }
    ...
    @media (max-width: 700px) {
        ...
        table.sat-table { min-width: 720px; }
        thead {
